...
CARBOM_Excel_file <- "C:/Users/vaulot/Google Drive/Papers/2016 Ribeiro CARBOM ISME/CARBOM Illumina results 2.1.xlsx"
...
# Read the sample table from an Excel file !!! The names of the columns are not changed in contrast to read.table
  samples<- read_excel(path = CARBOM_Excel_file, sheet = "Samples with NGS data", range = "A1:T56" )
...
# Read the otu table from an Excel file !!! The names of the columns are not changed in contrast to read.table
  otu_table<- read_excel(path = CARBOM_Excel_file, sheet = "18S OTU samples" )
...
# --- Combine 18S and nifH into a single matrix (all OTUs ) ---
  
  otu_combined_matrix <- merge (otu_matrix, otu_nifH_matrix_abund, by="row.names")
  row.names(otu_combined_matrix)=otu_combined_matrix$Row.names
  otu_combined_matrix=select(otu_combined_matrix, -Row.names)   
# remove samples for which no nifH (8 samples have no nifH) - NOT TO BE DONE because done before
#  sample_no_nifH <- names(which(sample_max_nifH == 0)) 
#  otu_combined_matrix_abund <- otu_combined_matrix_abund[-which(rownames(otu_combined_matrix_abund) %in% sample_no_nifH), ]
# --- Export combined matrix for use with sparCC
  write.table(t(otu_combined_matrix), file="carbom_18S_nifH_combined.txt", sep="\t", row.names = TRUE, quote=FALSE)
...
# Read SparCC correlation 
  ...
  # sparcc <- read.table("carbom_18S_nifH_combined_sparcc.txt",header=TRUE,sep="\t")
...
# Read SparCC pseudo_pvalues 
  ...
  # sparcc_pvalue <- read.table("carbom_18S_nifH_combined_sparcc.pvals_two_sided.txt",header=TRUE,sep="\t")
